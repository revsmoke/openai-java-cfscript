...
private constructor(
    ...
    internal fun validity(): Int =
        (if (id.asKnown().isPresent) 1 else 0) +
            ...
            object_.let { if (it == JsonValue.from("thread.run.step")) 1 else 0 } +
            (if (runId.asKnown().isPresent) 1 else 0) +
            (status.asKnown().getOrNull()?.validity() ?: 0) +
            (stepDetails.asKnown().getOrNull()?.validity() ?: 0) +
    ...
    private constructor(
        ...
        internal class Deserializer : BaseDeserializer<StepDetails>(StepDetails::class) {
            ...
            override fun ObjectCodec.deserialize(node: JsonNode): StepDetails {
                ...
                when (type) {
                    "message_creation" -> {
                        return tryDeserialize(node, jacksonTypeRef<MessageCreationStepDetails>())
                            ?.let { StepDetails(messageCreation = it, _json = json) }
                            ?: StepDetails(_json = json)
                    }
                    "tool_calls" -> {
                        return tryDeserialize(node, jacksonTypeRef<ToolCallsStepDetails>())?.let {
